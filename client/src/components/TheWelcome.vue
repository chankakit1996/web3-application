...
const { connectWallet, checkWalletIsConnected, sendTransactions, formData } =
  useCryptoStore()
...
const submitForm = async (e: Event) => {
  ...
  if (addressFrom && addressTo && amount && keyword && message) {
  ...
  <div class="flex w-full justify-center items-center">
    <div class="flex mf:flex-row flex-col items-start justify-between md:p-20 py-12 px-4">
      <div class="flex flex-1 justify-start items-start flex-col mf:mr-10">
        ...
        <button type="button" @click="connectWallet" v-if="accounts.length === 0"
        ...
        <div class="grid sm:grid-cols-3 grid-cols-2 w-full mt-10">
          <div
            ...
            }" v-for="(text, index) in welcomeBoxText" :key="index">
            {{ text }}
          </div>
        </div>
      </div>
      <div class="flex flex-col flex-1 items-center justify-start w-full mf:mt-0 mt-10">
        <div
          ...
          <div class="flex justify-between flex-col w-full h-full">
            ...
            <div>
              <p class="w-1/3 text-white font-light text-sm text-ellipsis overflow-hidden" :title="address">
                {{ address }}
              </p>
        ...
        <div class="p-5 sm:w-96 w-full flex flex-col justify-start items-center blue-glassmorphism">
          <input v-for="walletInput in walletInputs" :key="walletInput.placeholder" :placeholder="walletInput.placeholder"
            :type="walletInput.type" step="0.0001" min="0" v-model="formData[walletInput.value]"
